 ...
 * |------------------------|---------------------|---------|
 * | title / label | title + content[{title, text}] | A2UI title/label → dataType.title，同时放入 content 对象 |
 * | content（字面量） | content[{title, text}] | 与 title 合并到同一对象：{ title, text } |
 * | content（DataBinding） | content[{title, text}] | 同上，每项 item[path] 映射 |
 ...
 *
 * ⚠️ eview-ui 本地副本：自定义图标（非 iconType 枚举）走真实路径 `/icons/<name>.svg`
 * （icon 名直接拼），不调 resolveIcon、不产 React DOM——eview-ui 的 icon 相关属性只接 URL。
 * iconType 枚举（success/error/default）保留不变。icon 是路径字符串 → data 无 JSX →
 * 循环 data 的 containsJSX 翻 false（data 走 state.js 纯 JSON）。svg 文件由 BuildTrees 的
...
import { iconNameToPath } from './icon-placeholder'
...
function extractFieldMap(item: any): ItemFieldMap {
  ...
  //   eview-ui：自定义图标走真实路径 /icons/<name>.svg（icon 名直接拼），不再 resolveIcon；
  //   iconType 枚举仍走下方 iconIsType 分支。hasJSX 不再置 true（icon 不产 JSX → data 无 JSX → containsJSX:false）。
  if (p.icon) {
    ...
    transform(node: any, ctx: TransformContext) {
      ...
      if (Array.isArray(children)) {
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          // icon 分发：iconType（枚举）vs icon（真实路径 /icons/<name>.svg）
          if (f.iconValue) {
            if (f.iconIsType) {
              item.iconType = f.iconValue
            } else {
              item.icon = iconNameToPath(f.iconValue)
            }
          } else if (f.iconField) {
            // DataBinding icon：iconType 检测枚举；icon 非枚举时给真实路径（containsJSX:false）
            ...
            item.icon = Value.computed({
              ...
              containsJSX: false,
              transform: (raw: any) => (typeof raw === 'string' && !ICON_TYPES.has(raw)) ? iconNameToPath(raw) : null,
            })
          }
              ...
              if (f.iconField) {
                ...
                if (typeof name === 'string') {
                  if (ICON_TYPES.has(name)) {
                    ...
                    dataItem.icon = iconNameToPath(name)
                  }
                }
              } else if (f.iconValue) {
                if (f.iconIsType) {
                  ...
                  dataItem.icon = iconNameToPath(f.iconValue)
                }
              }
