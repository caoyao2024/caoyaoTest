/**
 * Collapse → Panel 映射（eview-ui 本地工厂副本）
 *
 * A2UI Collapse → eview-ui Panel 组件。
 *
 * ## 与 eview-react/Collapse.ts 的差异
 *
 * eview-react 的 Panel 是 **default 导出**（`import Panel, { PanelItem } from '@nce/eview-react/Panel'`），
 * 故 eview-react 工厂用字符串 import（= default）。
 * eview-ui 的 Panel 是 **named 导出**（`import { Panel, PanelItem } from '@cloudsop/eview-ui/Panel'`），
 * 无 default 导出，故此处改为 `import: { source, named: true }`（named）。
 * CollapseItem 复用 eview-react 工厂（其 import 本就是 named，eview-ui 一致，无需副本）。
 * 同源（`${pkg}/Panel`）的 named import 经 import-collector 合并为一行：
 *   `import { Panel, PanelItem } from '@cloudsop/eview-ui/Panel'`。
 *
 * transform 逻辑与 eview-react 完全一致（Panel API 相同），此处原样复用。
 *
 * ## Props 对照
 *
 ...
 *
 * 工厂化：接收目标组件库包名 `pkg`，构建 import 路径。
 */
...
export function createCollapseMapping(pkg: string): MappingDef {
  return {
    tag: 'Panel',
    // eview-ui 的 Panel 是 named 导出（无 default），与 eview-react 的 default 导出不同。
    // CollapseItem（复用 eview-react 工厂）也是 named、同源 → 合并成
    //   `import { Panel, PanelItem } from '@cloudsop/eview-ui/Panel'`。
