/**
 * convertTailwindToCSS.ts — CLI 模式的本地 tailwind→CSS/LESS 转换器
 *
 * 与 Electron 主进程 main/tailwind-to-css 模块导出同名同接口的函数/类型（单文件镜像）。
 *
 * 内部使用 tailwindcss v4 的 __unstable__loadDesignSystem API + 内联 tailwind 主题配置。
 *   - tailwindConfig 内联于本文件（与 Electron main 版保持单文件一致，不拆 tailwind.config.ts）
 *   - index.css 通过 readFileSync 加载（CLI/Node 环境；Electron 版用 Vite ?raw import）
 *
 * 接口层次：
 *   - convertTailwindToCSS(cn, useVar=false) → 扁平 base（公用兼容；UXAI 其他调用方不传 useVar 走默认，
 *     jsonToCode 这边调 style-converter 传 true）。返回扁平 camelCase 属性，承载不了 @media/伪类，仅取 base。
 *   - convertTailwindToCSSGrouped(cn, useVar) → { base, variants[] }（分组解析，含伪类/媒体查询，不折叠选择器）
 *   - convertTailwindToLessRule(cn, selector, opts) → LessRule（给 style-converter 直接产含 variants 的规则）
 *   - generateLessContent(rules) → LESS 文本（支持 variant 嵌套 @media）
 *
 * 响应式 variant（伪类 / @media / dark / rtl）由 parseCssToChunks 的嵌套帧栈统一解析；
 * useVar=true 输出 var(--xxx) 形式（保留运行时主题可覆盖），false 完全递归解析为具体值。
 *
 * 整个 lib/ 模块归属于 CLI 模式，api/ 文件夹中不保留。
 */
  ...
  theme: {
    extend: {
      colors: {
        ...
        "primary-container": "var(--color-info-primary-subtler, #EEF3FE)",
        ...
        "on-primary-fixed-variant": "var(--color-text-inverse, #FFFFFF)",
        ...
        "surface-variant": "var(--color-bg-1, #F3F3F3)",
        ...
        "inverse-on-surface-variant": "var(--color-text-inverse-disabled, #FFFFFF)",
        ...
        "info": "var(--color-info-primary, #2070F3)",
        "on-info": "var(--color-text-inverse, #FFFFFF)",
        "info-container": "var(--color-message-bg-info, #EEF3FE)",
        "on-info-container": "var(--color-text-primary, #191919)",
        "content-placeholder": "var(--color-text-placeholder, #AEAEAE)",
        "content-disabled": "var(--color-text-disabled, #C9C9C9)",
        "content-inverse-disabled": "var(--color-text-inverse-disabled, #FFFFFF)",
        "interactive-link": "var(--color-link, #0067D1)",
        "interactive-link-hover": "var(--color-link-hover, #2E86DE)",
        "interactive-link-active": "var(--color-link-active, #004EA8)",
        "interactive-link-visited": "var(--color-link-visited, #715AFB)",
        "interactive-link-disabled": "var(--color-link-disabled, #8ABEF3)",
        "scrim": "var(--color-bg-mask, rgba(25, 25, 25, 0.3))",
        "outline": "var(--color-border, #C9C9C9)",
        "outline-variant": "var(--color-border-separator, #DFDFDF)",
        "divider": "var(--color-border-separator, #DFDFDF)",
        "focus-ring": "var(--color-border-focus, #0067D1)",
        ...
        "hui-bg-mask": "var(--color-bg-mask, rgba(25, 25, 25, 0.3))",
        "hui-hover": "var(--color-hover, rgba(25, 25, 25, 0.05))",
        "hui-select": "var(--color-select, #E6F2FD)",
        "hui-fill": "var(--color-fill, rgba(25, 25, 25, 0.05))",
        "hui-fill-subtle": "var(--color-fill-subtle, #FFFFFF)",
        "hui-fill-disabled": "var(--color-fill-disabled, #DFDFDF)",
        "hui-fill-disabled-subtle": "var(--color-fill-disabled-subtle, rgba(25, 25, 25, 0.05))",
        ...
        "hui-icon-disabled": "var(--color-icon-disabled, #C9C9C9)",
        ...
        "hui-card-gray-disabled": "var(--color-card-gray-disabled, rgba(174, 174, 174, 0.05))",
        ...
        "hui-tag-bg-purple": "var(--color-tag-bg-purple, #E8CFFE99)",
      ...
      boxShadow: {
        ...
        base: "var(--shadow-base, 0 4px 12px 0 rgba(0, 0, 0, 0.16))",
        md: "var(--shadow-md, 0 8px 24px rgba(0, 0, 0, 0.08))",
        lg: "var(--shadow-lg, 0 8px 24px 0 rgba(0, 0, 0, 0.16))",
        xl: "var(--shadow-xl, 0 16px 48px 0 rgba(0, 0, 0, 0.16))",
        card: "var(--shadow-sm, 0px 1px 6px 0 rgba(0, 0, 0, 0.08))",
 ...
 * 公用方法：UXAI 其他调用方不传 useVar 走默认 false；jsonToCode style-converter 调用传 true。
 */
