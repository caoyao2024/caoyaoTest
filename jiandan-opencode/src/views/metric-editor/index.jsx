...
const toSelectOptions = (opts) => opts.map((o) => ({ value: o.value, text: o.label }));
...
function MetricEditor() {
  ...
  const handleSuccess = (values) => {
    saveMetric(values);
    message.success(isEdit ? "指标定义已更新" : "指标定义已创建");
  };
    ...
    >
      ...
      >
        ...
        <Form.Item label="指标分类" name="category" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(CATEGORY_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        <Form.Item label="统计周期" name="cycle" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(CYCLE_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>

        <h3 className="metric-editor__section-title">统计口径</h3>
        <Form.Item label="统计维度" name="dimension" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(DIMENSION_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        <Form.Item label="统计单位" name="unit" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(UNIT_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        ...
        <Form.Item label="指标极性" name="polarity" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(POLARITY_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        <Form.Item label="数据来源" name="source" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(SOURCE_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        <Form.Item label="责任人" name="owner" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(OWNER_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
        ...
        <Form.Item label="指标状态" name="status" rules={[{ required: true }]}>
          <Select style={FULL_WIDTH} options={toSelectOptions(STATUS_OPTIONS)} defaultLabel="请选择" enableClear />
        </Form.Item>
