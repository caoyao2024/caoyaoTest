...
|------|-----------|---------|
| `@ant-design/icons` | `@nce/icon-plus` 按需引入 | `import { IconPlusIcPublicSearch } from '@nce/icon-plus'`；`type="filled"` 换风格；`iconColor` 换色（支持 CSS 变量）；`iconSize` 只能取 12/14/16/20/24/32/36/40/48/60；icon+ 名**默认靠 skill 自带 `icons/icon-plus-names.json` 离线匹配**（方案 C，`Public`>`Ict` 优先；见 [source-project-guidelines §3.2](source-project-guidelines.md)）；在线 `getIconInfo` 名发现为备选 B（§3.4，外网不便时不用） |
