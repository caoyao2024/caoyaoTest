...
> **图标尺寸落在内层 `<IconPlusIc* />` 的 `iconSize`，不是 Button 的 `size`**（`size` 是按钮尺寸 normal/large/small）。源 antd `icon={<.../>}` 带尺寸时，把尺寸吸附到 `iconSize`（仅 12/14/16/20/24/32/36/40/48/60，就近吸附、默认 16）写到内层 icon+ 元素上：
>
> ```tsx
> // antd 源：<Button icon={<Icon name="save" size={14} />} ...>保存</Button>
> <Button status="primary" text="保存" leftIcon={<IconPlusIcPublicSave iconSize={14} />} onClick={handleSave} />
...
<IconButton iconName={<IconPlusIcPublicRefresh iconSize={14} />} tipText="刷新" size="small" onClick={handleRefresh} />
```

> **图标尺寸别丢**：源 `size={15}` 不是丢掉，而是吸附到 `iconSize={14}` 写到**内层 `<IconPlusIcPublicRefresh />`** 上（仅 12/14/16/20/24/32/36/40/48/60，就近吸附、默认 16）。`IconButton` 自身的 `size="small"` 是**按钮**尺寸，与图标尺寸是两回事，**不要**把图标尺寸塞到 IconButton 的 `size` 上。antd `@ant-design/icons` 没有 `size` prop、尺寸走 `style.fontSize`，同样剥 `px` 取数字吸附到内层 `iconSize`。
