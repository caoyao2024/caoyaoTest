...
const CATEGORY_OPTIONS = [
  { value: '资源性能', label: '资源性能' },
  { value: '容量规划', label: '容量规划' },
...
const SOURCE_OPTIONS = [
  { value: '指标中台', label: '指标中台' },
  { value: 'Prometheus', label: 'Prometheus' },
  { value: '日志平台', label: '日志平台' },
  { value: '业务库直连', label: '业务库直连' },
  { value: '手工填报', label: '手工填报' },
];

const OWNER_OPTIONS = [
  { value: '张明', label: '张明 · 平台架构组' },
  { value: '李慧', label: '李慧 · 基础资源组' },
  { value: '王涛', label: '王涛 · 数据平台组' },
  { value: '赵磊', label: '赵磊 · 容量规划组' },
  { value: '陈晓', label: '陈晓 · 网络运维组' },
  { value: '刘洋', label: '刘洋 · 服务保障组' },
  { value: '孙倩', label: '孙倩 · 运维支撑组' },
  { value: '周航', label: '周航 · 业务运营组' },
  { value: '何静', label: '何静 · 增长分析组' },
  { value: '马俊', label: '马俊 · 安全合规组' },
];

const POLARITY_OPTIONS = [
  { value: 'higher', label: '正向指标（越大越好）' },
  { value: 'lower', label: '逆向指标（越小越好）' },
];
...
function nextMetricId(list) {
  return 'MT-' + (1000 + list.length + 1);
}

const INITIAL_METRICS = RAW_METRICS.map((item) => decorateMetric(item));

export {
  computeRatio, isBreach, formatMetricValue, decorateMetric, nowStamp, nextMetricId,
  CATEGORY_OPTIONS, CYCLE_OPTIONS, DIMENSION_OPTIONS, UNIT_OPTIONS, SOURCE_OPTIONS,
  OWNER_OPTIONS, POLARITY_OPTIONS, STATUS_OPTIONS, STATUS_MAP, INITIAL_METRICS,
};
